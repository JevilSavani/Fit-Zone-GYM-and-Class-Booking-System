...
function MyBookingsPage() {
    ...
    return <main className="page-shell"><p className="eyebrow">YOUR TRAINING / 03</p><h1>{user?.name}'s bookings</h1>{loading && <p>Loading bookings...</p>}{error && <p className="error">{error}</p>}{!loading && !error && bookings.length === 0 && <p className="muted">No bookings yet. Your next session belongs on the calendar.</p>}{!loading && !error && bookings.map((booking) => <article className="booking-row" key={booking._id}><div><strong>{booking.className}</strong><span>{booking.trainerId?.name} / {booking.trainerId?.specialization}</span></div><div><span>{booking.date}</span><span>{booking.timeSlot}</span></div><b className={`status ${booking.status}`}>{booking.status}</b></article>)}</main>;
}
